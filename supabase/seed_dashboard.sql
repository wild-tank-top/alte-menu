-- =====================================================
-- Supabase Dashboard の SQL Editor にそのまま貼り付けて実行
-- =====================================================
...
(
  '帆立貝のポッシェ', 'Poached Scallop',
  '北海道産帆立貝', 'ブールブランソース、キャビア',
...
(
  'フォアグラのポワレ', 'Pan-seared Foie Gras',
  'フランス産フォアグラ', 'ポルト酒のソース、バルサミコのリダクション',
...
(
  '牛フィレのロティ', 'Roasted Beef Tenderloin',
  '国産牛フィレ肉', 'ペリグーソース、季節の野菜',
  '最高級部位を理想の火入れで',
...
(
  'ショコラのフォンダン', 'Chocolate Fondant',
  'ヴァローナ チョコレート', 'アングレーズソース、バニラアイスクリーム',
...
-- 3. course_menu_items
INSERT INTO course_menu_items (course_id, menu_item_id, section, sort_order)
SELECT c.id, m.id, x.section, x.sort_order
FROM (VALUES
  -- lunch_B
  ('lunch_B', 'アミューズ・ブッシュ', 'amuse',    1),
  ('lunch_B', '帆立貝のポッシェ',     'entree_1', 2),
  ('lunch_B', '鮮魚のロティ',         'fish',     3),
  ('lunch_B', 'ショコラのフォンダン', 'dessert',  4),
  -- lunch_EP
  ('lunch_EP', 'アミューズ・ブッシュ', 'amuse',    1),
  ('lunch_EP', 'フォアグラのポワレ',   'entree_1', 2),
  ('lunch_EP', '帆立貝のポッシェ',     'entree_2', 3),
  ('lunch_EP', '牛フィレのロティ',     'meat',     4),
  ('lunch_EP', 'ショコラのフォンダン', 'dessert',  5),
  -- dinner_AL
  ('dinner_AL', 'アミューズ・ブッシュ', 'amuse',    1),
  ('dinner_AL', '帆立貝のポッシェ',     'entree_1', 2),
  ('dinner_AL', '鮮魚のロティ',         'fish',     3),
  ('dinner_AL', '牛フィレのロティ',     'meat',     4),
  ('dinner_AL', 'ショコラのフォンダン', 'dessert',  5),
  -- dinner_SP
  ('dinner_SP', 'アミューズ・ブッシュ', 'amuse',    1),
  ('dinner_SP', 'フォアグラのポワレ',   'entree_1', 2),
  ('dinner_SP', '帆立貝のポッシェ',     'entree_2', 3),
  ('dinner_SP', '鮮魚のロティ',         'fish',     4),
  ('dinner_SP', '牛フィレのロティ',     'meat',     5),
  ('dinner_SP', 'ショコラのフォンダン', 'dessert',  6)
) AS x(slug, item_name, section, sort_order)
JOIN courses    c ON c.slug = x.slug
JOIN menu_items m ON m.name = x.item_name
ON CONFLICT (course_id, menu_item_id) DO NOTHING;
...
('ソーヴィニヨン・ブラン',   'Sauvignon Blanc',             'white',    'light',  'high',   NULL,   'グレープフルーツ、ハーブ、草',           'シャープな酸味と爽やかな香り。シーフード、山羊チーズに好相性。',     'ロワール（フランス）、ニュージーランド'),
...
('シャンパーニュ',           'Champagne (Blanc de Blancs)', 'sparkling', 'medium', 'high',  NULL,   '白い花、レモン、クリーム、トースト',     '繊細な泡と生き生きとした酸味。食前酒やシーフードに万能。',           'シャンパーニュ地方（フランス）');
